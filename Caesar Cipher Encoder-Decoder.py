...
def caesar(message, offset):   # Define a custom function, indent all following blocks. Use the parameters message and offset for reusability
    alphabet = 'abcdefghijklmnopqrstuvwxyz' # Alphabet for cipher
    encrypted_text = ''   # String to store the encrypted text
    for char in message.lower():  # Loop to go through every letter sequentially. Requires an indented block. P.S. Specified to go for lowercase version
        if char == ' ':  # Check if the character is equal to a space using the == boolean operator
            encrypted_text += char  # Add the aforementioned space to our encrypted text, if `char` is a space
        else:   # If `char` is not a space
            index = alphabet.find(char) # Find the index of `char` in the alphabet and assign to a variable
...
caesar(text, shift)    # Call on our function using parameters `text` for message and `shift` for offset
